...
def graph_ranks(avranks: list[float], names: list[str], lowv: int = None, highv: int = None,
    ...
    for i in range(math.ceil(k / 2)):
        ...
        line([(rankpos(ssums[i]), cline),
             ...
             linewidth=linewidth)
        if labels:
            text(textspace + 0.3, chei - 0.1, format(ssums[i], '.1f'), ha="right", va="center", size=15)
    ...
    for i in range(math.ceil(k / 2), k):
        ...
        line([(rankpos(ssums[i]), cline),
             ...
             linewidth=linewidth)
        if labels:
            text(textspace + scalewidth - 0.3, chei - 0.1, format(ssums[i], '.1f'), ha="left", va="center", size=15)
